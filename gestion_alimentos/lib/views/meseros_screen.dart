...
class MeserosScreen extends StatelessWidget {
              ...
              child: GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    onTap: () {
                      if (status == 'Comiendo') {
                        _showRequestBillDialog(context, mesaNumber, viewModel);
                      } else {
  ...
  void _showRequestBillDialog(BuildContext context, int mesaNumber, MeseroViewModel viewModel) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Pedir Cuenta Mesa $mesaNumber'),
          content: const Text('¿Desea pedir la cuenta para esta mesa?'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
            ...
            TextButton(
              onPressed: () async {
                await viewModel.markMesaAsCleaning(mesaNumber);
                Navigator.of(context).pop();
              },
